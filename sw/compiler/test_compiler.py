...
def test_codegen():
    ...
    # Use sequential scheduling (no fusion) for basic codegen test
    from scheduler.scheduler import ScheduleConfig, ScheduleStrategy
    config = ScheduleConfig(strategy=ScheduleStrategy.SEQUENTIAL, enable_fusion=False)
    scheduler = Scheduler(schedule_config=config)
    schedule = scheduler.schedule(graph)
